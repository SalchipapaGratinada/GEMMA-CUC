...
namespace Gemma.Cadenas
{
    public class CdEstudiantes
    {
        ...
        public static string cargarDropClasesConIdEstudiante(int idEstudiante)
        {
            string cd = "SELECT `classes`.`id`, `classes`.`name`, (SELECT `users`.`user_name` FROM " +
                "`users` WHERE `users`.`id` = `class_students`.`teacher_id`) AS 'teacher_name', (SELECT `users`.`user_lastname` " +
                "FROM `users` WHERE " +
                "`users`.`id` = `class_students`.`teacher_id`) AS ' teacher_lastname' " +
                "FROM `class_students` INNER JOIN `users` ON `users`.`id` = `class_students`.`users_id` " +
        ...
        {
            string cd = "SELECT `users`.`user_name`,`wallets`.`summary` AS 'summary' ," +
                " `groups`.`name` AS 'group_name',`classes`.`name` AS 'class_name' " +
                "FROM `wallets` " +
                ...
                "INNER JOIN `groups_members` ON `groups_members`.`groups_classes_id`=`classes`.`id` AND `groups_members`.`users_id`=`users`.`id` " +
                "INNER JOIN `groups` ON `groups`.`id`=`groups_members`.`groups_id`; ";
            return cd;
        }
